<?php
  ...
  $select = $conec->prepare("SELECT id_chamado, ds_status, ds_chamado, ds_prioridade FROM `tb_chamado` WHERE ds_status = 'ABERTO'");
  $select->execute();
?>
<HTML>
<HEAD>
					...
					<tbody>
						<?php while($chamado = $select->fetch()) { ?>
							<tr>
								...
								<td><?php echo "<a href='frmFecharChamado.php?id=".$chamado['id_chamado']."'>Verificar</a>" ?></td>
